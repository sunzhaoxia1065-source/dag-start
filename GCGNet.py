...
class GCGNet(ModelBase):
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                pred_dim = config.pred_dim if config.pred_dim is not None else series_dim
                target = target[:, -self.model_pred_len:, :pred_dim]
                output = output[:, -self.model_pred_len:, :pred_dim]
                if (
    ...
    def forecast_fit(
        ...
        for epoch in range(config.num_epochs):
            ...
            if train_ratio_in_tv != 1:
                ...
                if np.isnan(valid_loss) or np.isinf(valid_loss):
                    logger.warning(f"Validation loss is nan/inf: {valid_loss}")
    ...
    def batch_forecast(
        ...
        pred_dim = self.config.pred_dim if self.config.pred_dim is not None else series_dim
        return answers[..., :pred_dim]
