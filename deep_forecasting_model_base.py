...
# pred_dim: number of endogenous variables to predict (default None = predict all series_dim).
#           Set to a value <= series_dim to only predict the first pred_dim endogenous variables.
...
class DeepForecastingModelBase(ModelBase):
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                target = target[:, -config.horizon :, :config.pred_dim]
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        # pred_dim: number of endogenous variables to predict.
        # Default to series_dim (predict all endogenous variables) for backward compatibility.
        if self.config.pred_dim is None:
            self.config.pred_dim = series_dim
        elif self.config.pred_dim > series_dim:
            raise ValueError(
        ...
        with torch.no_grad():
            ...
            while answer is None or answer.shape[0] < horizon:
                ...
                if answer.shape[0] >= horizon:
                    ...
                    return answer[-horizon:, :config.pred_dim]
    ...
    def batch_forecast(
        ...
        return answers[..., :self.config.pred_dim]
